...
{
    #STARTDIR="/data"
    STARTDIR="/home/polar/CODE/VM/CLONED/Partclone"
    EXTENSION="*.gz"
    filebrowser "Filebrowser" "$STARTDIR" "$EXTENSION"
    log "Selected image: $FILE_SELECTED"
    log "Selected image path: $FILE_SELECTED_PATH"
    if [ ! -z $FILE_SELECTED ]; then
        infobox "Flash" "I am going to flash the selected file to the system partition:\n $FILE_SELECTED "
        flash_null $FILE_SELECTED_PATH && infobox "Flashing successful. I am going to reboot now!" && reboot
    fi         
}
...
function info_flash_sucess()
{
    infobox "Flashing was successful."
    reboot
}
